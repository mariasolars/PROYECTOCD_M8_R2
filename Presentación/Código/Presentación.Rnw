...
Hay diferencias significativas según el género en el tiempo medio dedicado a las noticias y la política (\emph{U} = \Sexpr{round(gnoticias$statistic, 2)}, \emph{p} = \Sexpr{round(gnoticias$p.value, 3)}) y el tiempo de uso de Internet (\emph{U} = \Sexpr{round(gtinternet$statistic, 2)}, \emph{p} = \Sexpr{round(gtinternet$p.value, 3)}).
...
<<echo=FALSE,fig=true,width=1.2,height=0.87>>=
#Correlaciones
...
corrplot(coredad$r, method="ellipse", col=colorRampPalette(c("dodgerblue3","white","red"))(10),
         ...
         cl.cex = 0.17, # Ajustar tamaño de las etiquetas de la leyenda
         number.cex = 0.2, # Ajustar tamaño de los coeficientes
         tl.cex = 0.17, # Ajustar tamaño de las etiquetas de las variable
...
<<echo=FALSE,fig=true,width=0.7,height=0.6>>=
ggplot(datosr2, aes(x = agea, y = netustm, color = agea)) +
  geom_point(size = 0.0001, alpha = 0.8, shape = 20) +  # Capa de puntos con tamaño personalizado
  geom_smooth(method = "lm", se = FALSE, color = "blue", linewidth = 0.1) +  # Línea de regresión lineal sin intervalo de confianza
  ...
  theme(
    ...
    axis.ticks.length = unit(0.2, "mm"),  # Longitud de las marcas de los ejes
    axis.title = element_text(size = 1.25),  # Tamaño de las etiquetas de los ejes
    axis.text = element_text(size = 1),  # Tamaño del texto de los ejes
...
corrplot(coredu$r, method="ellipse", col=colorRampPalette(c("dodgerblue3","white","red"))(10),
         ...
         # hide correlation coefficient on the principal diagonal
         cl.cex = 0.17, # Ajustar tamaño de las etiquetas de la leyenda
         number.cex = 0.25, # Ajustar tamaño de los coeficientes
         tl.cex = 0.17, # Ajustar tamaño de las etiquetas de las variable
         diag=F,mar = c(0, 0, 0, 0))
@
\end{figure}
